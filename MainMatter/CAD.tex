...
\chapter{CAD Deployment for Indoor WSNs}
\label{cap:cad}

Over the last years, many smart buildings applications, such as indoor localization or safety systems, have been subject of intense research. Smart environments usually rely on several hardware nodes equipped with sensors, actuators and communication functionalities.
...
The tool\footnote{A video demo of the tool has been published at \url{https://youtu.be/6c6D6wolDBQ}}
...
\caption{Overview of the application stack. The script interpreter features standard ECMAScript functionality and on top of that provides additional classes from the Qt API, QCAD API and the \emph{SmartBuilding} module.}
\end{figure}
...
\caption{Functional overview of the system components. Drawing tools and algorithms for systems deployment and simulation are extensible through ECMAScript or C++ plug-ins.}
\end{figure}

\begin{figure}
\centering\includegraphics[scale=0.4]{floorplan2.pdf}
\caption{Floor-plan design tool. User can specify the layout of the rooms and a possible set of candidate sites for the node placement.}
\label{fig:floor}
\end{figure}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.5]{main.pdf}
\caption{System Process. After the design of the floor plan, different parameters are used to define the search for an optimal allocation of nodes.}
\label{fig:main}
\end{figure}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.65]{coverage.pdf}
\caption{Sample floor-plans with a location $l$ covered (a) in single mode, (b) for trilateration, and (c) for fingerprinting where $rss_{l,1} < rss_{l,2}$.}
\label{fig:coverage}
\end{figure}
...
\begin{figure}
\centering\includegraphics[scale=0.45]{example.pdf}
...
\begin{table}[h!tb]
\centering
\caption{Values of coefficients depending on LOS (Line-Of-Sight) or NLOS (Non-Line-Of-Sight) propagations. Values have been taken from the WINNER II path loss model \cite{Kyosti2008}.}
\label{tab:pathloss}
...
\begin{tabular}{ | l | l |}
    ...
    $k$-coverage & number of ref. nodes required by the system \\ \hline
    $k_l$ & current number of ref. nodes covering $l$\\ \hline
    $S$ & min. signal space Euclidean distance threshold \\ \hline
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.75]{rssdiv.pdf}
...
The decision variable $x_{jt}=1$ represent the allocation of a node of type $t$ in location $j$; $a_{ljt}$ is equal to $1$ if location $l$ can be reached by a node of type $t$ placed in $j$, and $a_{ljt}=0$ otherwise. $y_l=1$ if location $l$ is covered, $y_l=0$ otherwise. The constraint \eqref{eq:one} set to one the maximum number of nodes that can be located in each site.
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.45]{exec_times.pdf}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.45]{greedy_vs_vns.pdf}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.45]{greedy_vs_vns2.pdf}
...
\begin{figure}
\centering\includegraphics[scale=0.37]{necst.pdf}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.45]{loc_error.pdf}
...
\begin{figure}[h!tb]
\centering\includegraphics[scale=0.45]{group_errors.pdf}
